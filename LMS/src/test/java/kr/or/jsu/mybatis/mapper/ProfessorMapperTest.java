...
//	@Test
	...
	@Test
	@DisplayName("교수 전체 목록 조회 테스트")
	void testSelectProfessorList() {
        // 1. 쿼리 실행 시 예외가 발생하지 않는지 검증 (실행 성공 여부 확인)
        assertDoesNotThrow(() -> mapper.selectProfessorList(), 
                           "쿼리 실행 중 예외가 발생했습니다 (SQL, 매핑 등 확인 필요).");

        // 2. 쿼리를 다시 실행하여 반환된 List 객체 자체가 null이 아닌지 검증 (객체 유효성 확인)
        assertNotNull(mapper.selectProfessorList(), "조회된 List 객체는 null이 아니어야 합니다.");

        // 3. 로그 출력 (선택 사항: 쿼리가 세 번째 실행되지만 코드가 간결함)
        log.info("===> 조회 결과 크기: {}", mapper.selectProfessorList().size());
        
//        assertEquals(0, (mapper.selectProfessorList()).size());
