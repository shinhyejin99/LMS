...
<%-- 	<link rel="stylesheet" href="${pageContext.request.contextPath}/css/potalBoard.css"> --%>
    ...
    <script src="${pageContext.request.contextPath}/js/app/portal/portalNoticeForm.js"></script>
...
<%@ include file="/WEB-INF/views/user/common/userNotificationScript.jspf" %>
    <div class="main-container">
        <h1 class="page-title">
             <c:choose>
                <c:when test="${not empty notice}">공지사항 정보 수정</c:when>
            	<c:otherwise>공지사항 정보 등록</c:otherwise>
            </c:choose>
        ...
        <c:choose>
		    <c:when test="${not empty notice}">
		        <c:url var="formAction" value="/portal/notice/modify/${notice.noticeId}"/>
		    </c:when>
		    <c:otherwise>
		        <c:url var="formAction" value="/portal/notice/create"/>
		    </c:otherwise>
        </c:choose>
        ...
        <form action="${formAction}"
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="title" name="title"
                       value="" placeholder="공지사항 제목을 입력하세요" required maxlength="200">
            </div>



            <div class="form-group form-check">
                <input type="checkbox" class="form-check-input" id="isUrgent" name="isUrgent" value="Y">
                 <c:if test="${notice.isUrgent == 'Y'}">checked</c:if>
                <label class="form-check-label" for="isUrgent">긴급 공지</label>
				<button type="button" class="btn btn-info-outline" id="fillUrgentNoticeBtn">📢 자동완성</button>
            </div>
...
<%--                                     <button type="button" class="btn-remove-file" data-file-id="${file.fileId}" data-file-order="${file.fileOrder}">[삭제]</button> --%>
<!--                                 </li> -->
<%--                             </c:forEach> --%>
<!--                         </ul> -->
<!--                     </div> -->
