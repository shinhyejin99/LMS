 ...
 *   수정일      			수정자           수정내용
 *  -----------   	-------------    ---------------------------
 * 2025. 10. 29.     	송태호            강의 신청 목록 조회 화면 스크립트
 *
 * </pre>
 */
...
(function() {
    ...
    // API 엔드포인트
    const API_URL = '/lms/api/v1/professor/lecture/apply';
    ...
    function init() {
        ...
        // 데이터 로드
        loadLectureApplyList();
    }

    /**
     * 강의 신청 목록 데이터 로드
     */
    async function loadLectureApplyList() {
        // 로딩 상태 표시
        showLoading();
        ...
        try {
            ...
            console.error('강의 신청 목록 로드 실패:', error);
            showError();
        }
    }
    ...
    function createTableRow(lecture) {
        ...
        if (lecture.approverName && lecture.approverRole) {
            approverCell.innerHTML = `${lecture.approverName}<br>(${lecture.approverRole})`;
        } else {
