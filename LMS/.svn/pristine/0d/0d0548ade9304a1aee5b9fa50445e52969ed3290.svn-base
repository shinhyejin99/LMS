...
function badgeForType(t) {
  if (t === 'NOTICE') return <span className="badge text-black border border-warning-subtle" style={{ backgroundColor: '#ffc107' }}>공지</span>
  if (t === 'MATERIAL') return <span className="badge text-black border border-info-subtle" style={{ backgroundColor: '#0dcaf0' }}>자료</span>
...
export default function Board() {
  ...
  const renderItem = (p, index) => {
    ...
    const href = `/classroom/professor/${encodeURIComponent(lectureId)}/board/${encodeURIComponent(String(targetId))}`
    return (
      <Link
        key={String(targetId)}
        to={href}
  ...
  const TabButton = ({ id, label, active, onClick, badge }) => {
    const badgeStyle = {
      all: { backgroundColor: '#6c757d', color: 'white' },
      notice: { backgroundColor: '#ffc107', color: 'black' },
      material: { backgroundColor: '#0dcaf0', color: 'black' },
    }[id] || { backgroundColor: '#6c757d', color: 'white' }

    return (
      <button className={`nav-link ${active ? 'active' : ''}`} id={`tab-${id}`} type="button" onClick={onClick} role="tab">
        {label} {badge != null && <span className="badge ms-1" style={badgeStyle}>{badge}</span>}
      </button>
    )
  }
            ...
            <>
              <div className="tab-pane fade show active p-0">
                ...
                <div className="mt-3 d-flex align-items-center">
                  ...
                  <Link id="btn-create" className="btn btn-sm btn-primary py-1 px-2" to={`/classroom/professor/${encodeURIComponent(lectureId)}/board/new`}>새 게시글</Link>
                </div>
              </div>
            </>
          )}
